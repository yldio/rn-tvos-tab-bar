...
- (void)setItems:(NSArray<NSString *> *)items {
  ...
  for (NSString *item in items) {
    UITabBarItem *tabBarItem =
        [[UITabBarItem alloc] initWithTitle:item
...
- (void)tabBar:(UITabBar *)tabBar didSelectItem:(UITabBarItem *)item {
  ...
  if (_eventEmitter != nullptr) {
    std::dynamic_pointer_cast<const facebook::react::YLDTabBarEventEmitter>(
        _eventEmitter)
        ->onSelectItem(facebook::react::YLDTabBarEventEmitter::OnSelectItem{
            .index = index});
  }
#else
...
- (void)updateProps:(facebook::react::Props::Shared const &)props
  ...
  const auto &oldScreenProps =
      *std::static_pointer_cast<const facebook::react::YLDTabBarProps>(_props);
  const auto &newScreenProps =
      *std::static_pointer_cast<const facebook::react::YLDTabBarProps>(props);
...
#ifdef RCT_NEW_ARCH_ENABLED
Class<RCTComponentViewProtocol> YLDTabBarCls(void) { return YLDTabBar.class; }
#endif
